...
class ProductxController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('productsx.index', compact('products'));
    }
    public function create()
    {
        $categories = Category::all();
        return view('productsx.create', compact('categories'));
    }
    public function save(ValidationProduct $validateProductRequest)
    {
        ...
        return redirect()->route('productsx.index')->with("msg", "Add product success")->with('msgAction', 'success');
    }
    ...
    public function edit($id = 0, Request $request)
    {
        ...
        if (!empty($id)) {
            ...
            if ($product == null) {
                return redirect()->route('productsx.index')->with("msg", "Product not found")->with('msgAction', 'error');
            } else {
                return view('productsx.edit', compact('product', 'categories'));
            }
        } else {
            return redirect()->route('productsx.index')->with("msg", "Id Product not valid")->with('msgAction', 'error');
        }
    }
    public function update(ValidationProduct $request, $id)
    {
        ...
        if (!empty($id)) {
            ...
            if ($product == null) {
                ...
                $this->productService->updateProduct($product);
                return redirect()->route('productsx.index')->with("msg", "Edit Product success")->with('msgAction', 'success');
            }
        } else {
            return redirect()->route('productsx.index')->with("msg", "Id Product not valid")->with('msgAction', 'error');
        }
